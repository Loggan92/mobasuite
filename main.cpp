...
void minecraft()
{
	*n[82] = '\0';
	...
	if (detect_x64())
	{
		pa(82, L"jdk-19.0.2_windows-x64_bin.exe");
		download(L"jdk-19.0.2_windows-x64_bin.exe", 82);
	}
	else
	{
		pa(82, L"jdk-8u361-windows-i586.exe");
		download(L"jdk-8u361-windows-i586.exe", 82);
	}
	if (detect_x64())
	{
		sei = {};
	...
	else
	{
		...
		std::filesystem::remove_all(n[0]);
		MessageBox(nullptr,
			L"Minecraft Launcher > Minecraft: Java Edition > Installations > Latest > Edit > More Options > Java Executable Path > <drive>:\\Program Files\\Java\\jre-8u361\\bin\\javaw.exe",
			L"Post-Install Instructions", MB_OK);
	}
